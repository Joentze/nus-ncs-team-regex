...
if __name__ == "__main__":
    df = pandas.read_csv("./backend/misc/station_names.csv")
    print(df.head(5))
    station_mapping = df.set_index('StationCode')['StationName'].to_dict()
    with open("./station_code_map.json", "w") as file:
        json.dump(station_mapping, file)
