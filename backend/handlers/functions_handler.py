...
class Augmented_Prompt():
    ...
    async def get_traffic_data(self):
        ...
        try:
            ...
            speedMessage, roadName, distance = traffic_finder.get_speed(
                self.coords)
    ...
    async def get_carpark_data(self):
        ...
        try:
            ...
            available_carparks = carpark_finder.get_top_5_closest_carparks(
                self.coords)
    ...
    async def get_message_data(self):
        ...
        try:
            ...
            traffic_incidents = message_finder.get_events(
                "TrafficIncidents", self.coords)
    ...
    async def get_bus_data(self):
        try:
            ...
            nearby_stops = busstop_finder.find_nearby_bus_stops(
                (float(coords[0]), float(coords[1])))
            crowded_stops = busstop_finder.find_crowded_stops(
                nearby_stops.head(5))
            ...
            less_crowded_alternatives = busstop_finder.find_less_crowded_alternatives(
                crowded_stop_code).head(3)
...
async def get_crowd_management_suggestions(problem: str) -> str:
    """returns prompt for chatgpt"""
    nlp = spacy.load("misc/NEP2")
    extracted_coords = identify_location(nlp, problem)
    if extracted_coords:
        a_prompt = Augmented_Prompt(extracted_coords, problem)
        prompt = asyncio.run(a_prompt.construct())
        chat_completion = await client.chat.completions.create(
            messages=[
                {"role": "system", "content": "You are a master traffic assistant"},
